...
/// @notice Synthetic, predictable prices for valueless BSC Testnet demo tokens only.
...
contract BscScenarioOracle is IPriceOracle {
    error DemoChainOnly();
    ...
    constructor(address quoteToken_, address[] memory tokens, uint256[] memory basePrices, uint64 cycleSeconds_) {
        _requireDemoChain();
        if (
            tokens.length == 0 || tokens.length > 8 || tokens.length != basePrices.length || cycleSeconds_ < 4 minutes
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ...
            uint8 tokenDecimals = BscTestToken(token).decimals();
            if (tokenDecimals != 18) revert InvalidConfiguration();
    ...
    function description() external pure returns (string memory) {
        return "YieldShield synthetic BSC Testnet demo prices";
    }
    ...
    function _requireDemoToken(address token, uint8 expectedDecimals) private view {
        if (
            token.codehash != keccak256(type(BscTestToken).runtimeCode)
                || BscTestToken(token).decimals() != expectedDecimals
        ) revert InvalidConfiguration();
    }
}
