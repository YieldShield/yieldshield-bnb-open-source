...
/// @notice BSC Testnet-only initialization adapter with explicit short demo waiting periods.
...
contract BscPoolInitializeModule is ProtocolAccessControlUpgradeable {
    error DemoChainOnly();
    error InvalidModule();
    ...
    fallback() external {
        ...
        bytes32 expectedTokenCode = keccak256(type(BscTestToken).runtimeCode);
        if (
            shielded.token.codehash != expectedTokenCode || backing.token.codehash != expectedTokenCode
                || BscTestToken(shielded.token).decimals() != 18
                || BscTestToken(backing.token).decimals() != 6
