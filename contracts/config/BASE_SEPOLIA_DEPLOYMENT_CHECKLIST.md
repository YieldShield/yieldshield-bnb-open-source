...
Each transaction intent, nonce and deterministic signed transaction hash is saved **before** submission. Two confirmations and runtime/postcondition checks follow each deployment. A retry signs the same saved request and requires the same hash; a changed intent, changed runtime, unexpected pending account transaction, or nonce consumed without the known receipt stops execution. Never delete a transaction entry merely to make the script continue.
