#!/usr/bin/env node
/** Reviewed, sequential BSC Testnet synthetic bootstrap. Default: prepare only; never broadcasts without --broadcast.
 * Secrets are read from ignored contracts/.env.bsc.local and are never serialized/logged.
...
function toRequest(value) {
  const req = { ...value };
  for (const key of ["gas", "gasPrice", "value"]) if (req[key] !== undefined) req[key] = BigInt(req[key]);
  return req;
}
...
export async function readCanonicalReceipt(client, hash, id) {
  ...
  assert(
    typeof head.number === "bigint" && head.number >= receipt.blockNumber + 9n,
    `${id}: ten sealed block confirmations required`,
  );
...
export class SequentialDeployment {
  ...
  async assertSubmissionLimits(request, additionalRequest = false) {
    ...
    assert(BigInt(request.gas) <= 16000000n, "Transaction exceeds conservative BSC gas cap");
    assert(BigInt(request.gasPrice) <= this.maxFeePerGas, "Transaction fee exceeds configured deployment cap");
    ...
    const reserved = Object.values(this.manifest.transactions).reduce(
      (n, t) => n + maximumExecutionCost(t.request),
      0n,
    );
    assert(
    ...
    assert(
      (await this.client.getBalance({ address: this.account.address })) >= maximumCost + parseEther("0.0001"),
      "Insufficient test BNB including gas reserve",
    );
  }
  async transaction(id, { to, data, kind = "call" }) {
    const intent = { chainId: CHAIN_ID, from: this.account.address, to: to ?? null, data, value: "0" },
    ...
    if (!entry) {
      ...
      assert(gas <= 16000000n, "Transaction exceeds conservative BSC gas cap");
      const gasPrice = await this.client.getGasPrice();
      const request = { chainId: CHAIN_ID, type: "legacy", nonce: latest, to, data, value: 0n, gas, gasPrice };
  ...
  async deploy(name, artifactName = name, args = []) {
    ...
    assert((a.deployedBytecode.object.length - 2) / 2 <= 24576, `${artifactName} exceeds BSC runtime limit`);
    ...
    assert((data.length - 2) / 2 <= 49152, `${name} exceeds BSC initcode limit`);
