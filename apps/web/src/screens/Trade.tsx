...
export function Trade() {
  ...
  const [toleranceBps, setToleranceBps] = useState(500);
  ...
  const limit = shownQuote ? demoTradeLimit(shownQuote, toleranceBps) : 0n;
  ...
  async function confirmTrade() {
    ...
    const result = await tx.submit({
      ...
      side: review.side,
      amount: review.amount,
      limit: demoTradeLimit(review, toleranceBps),
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="min-w-0">
          ...
          <dl className="mt-6 space-y-3 border-y border-hairline py-5 text-[14px]">
            ...
            />
            <TradeRow label="Network fee" value="Shown in wallet · test BNB" />
          </dl>
          <fieldset className="mt-5 rounded-input border border-brand/30 bg-brand-tint/40 p-4">
            <legend className="px-1 text-[13px] font-bold text-ink">Maximum price movement you accept</legend>
            <div className="flex flex-wrap gap-2" role="group" aria-label="Price tolerance">
              {[200, 500, 1000, 2000].map((bps) => (
                <button
                  key={bps}
                  type="button"
                  ...
                  className={`min-h-11 rounded-input px-4 text-[13px] font-bold ${toleranceBps === bps ? "bg-ink text-white" : "border border-hairline bg-white text-body hover:border-brand"}`}
                >
                  {bps / 100}%
                </button>
              ))}
        ...
        <div className="min-w-0 space-y-5">
          <Card>
            ...
            <Link to="/positions" className="mt-5 inline-block text-[14px] font-bold text-brand-deep">
