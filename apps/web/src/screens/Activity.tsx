...
const META: Record<ActivityVM["kind"], { label: string; inflow: boolean; Icon: typeof PlusIcon }> = {
  ...
  activate: { label: "Activated protection", inflow: true, Icon: ShieldIcon },
...
export function Activity() {
  ...
  return (
    <div className="animate-fade-up">
      <h1 className="mb-5 text-[26px] font-extrabold tracking-tight2 md:text-[30px]">Activity</h1>
                    ...
                    <tr key={e.txId} className="border-b border-hairline last:border-0 hover:bg-subtle">
                      ...
                      <td
                        className={cn(
                          ...
                          inflow ? "text-green-dark" : "text-ink",
                        )}
                      >
                        {amt ? `${inflow ? "+" : ""}${amt}` : "—"}
                      </td>
...
function Link({ e, className }: { e: ActivityVM; className?: string }) {
  ...
  return (
    <a href={chain.explorerTxUrl(e.txId)} target="_blank" rel="noreferrer" className={className}>
      ...
      {amt && (
        <div className={cn("text-[14.5px] font-bold tnum", inflow ? "text-green-dark" : "text-ink")}>
          {inflow ? "+" : ""}
          {amt}
        </div>
      )}
