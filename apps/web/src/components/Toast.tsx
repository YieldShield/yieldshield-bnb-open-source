      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-28 z-50 flex flex-col items-center gap-2 px-4 md:bottom-8">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={cn(
              "pointer-events-auto flex max-w-[420px] items-center gap-2.5 rounded-input px-4 py-3 text-[14px] font-semibold shadow-panel animate-fade-up",
              t.kind === "success" && "bg-ink text-white",
              t.kind === "error" && "bg-amber-tint text-amber-deep",
              t.kind === "info" && "bg-surface text-ink",
            )}
          >
            {t.kind === "success" && <CheckIcon className="h-4.5 w-4.5 text-green-bright" />}
