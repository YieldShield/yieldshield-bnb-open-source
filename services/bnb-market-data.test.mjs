  ...
  return {
    ...
    multicall: async ({ blockNumber, contracts }) => {
      ...
      return ASSETS.flatMap((a) => [
        ...
        success(8), success(a.description),
      ]);
    },
    ...overrides,
  };
...
test("rejects the wrong chain, stale/future blocks and changed block identities", async () => {
  ...
  for (const offset of [-121, 1]) {
    await assert.rejects(readSourceSnapshot(client({ getBlock: async () => ({ number: 1000n, timestamp: BigInt(timestamp + offset), hash: `0x${"a".repeat(64)}` }) }), { now }), /block/);
  }
  let reads = 0;
  await assert.rejects(readSourceSnapshot(client({ getBlock: async () => ({ number: 1000n, timestamp: BigInt(timestamp - 5), hash: `0x${(++reads === 1 ? "a" : "b").repeat(64)}` }) }), { now }), /changed/);
});
test("bad individual feeds cannot enable a scenario or disable healthy tokens", async () => {
  const original = client();
  for (const mutation of [
    (rows) => { rows[0] = { status: "failure" }; },
    (rows) => { rows[0].result[1] = 0n; },
    (rows) => { rows[0].result[1] = -10n; },
    (rows) => { rows[0].result[3] = BigInt(timestamp + 2); },
    (rows) => { rows[0].result[4] = 1n; },
    (rows) => { rows[1] = success(18); },
    (rows) => { rows[2] = success("BTC / USD"); },
  ]) {
    const c = client({ multicall: async (args) => { const rows = await original.multicall(args); mutation(rows); return rows; } });
...
test("cached reads preserve oracle age and expire at the source-block deadline", async () => {
  const original = client();
  const snapshot = await readSourceSnapshot(client({ multicall: async (args) => {
...
function response() {
  return { statusCode: 0, headers: {}, body: "", setHeader(k, v) { this.headers[k] = v; }, end(body) { this.body = body; } };
}
test("HTTP responses deduplicate concurrent reads, never allow POST or expose RPC errors", async () => {
  let reads = 0;
  const original = client();
  const handler = createMarketHandler(client({ multicall: async (args) => { reads++; return original.multicall(args); } }), { now });
  ...
  await createMarketHandler(client({ getChainId: async () => { throw new Error("secret-rpc-url"); } }), { now })({ method: "GET" }, error);
