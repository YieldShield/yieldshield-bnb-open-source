...
function validateBlock(block, now) {
  if (
    ...
    block.timestamp > BigInt(now) ||
    BigInt(now) - block.timestamp > BigInt(MAX_BLOCK_AGE_SECONDS)
  ) {
...
function parseRound(result, decimalsResult, descriptionResult, asset, block) {
  ...
  if (
    ...
    round[1] <= 0n ||
    round[1] >= 2n ** 255n ||
    typeof round[2] !== "bigint" ||
    round[2] <= 0n ||
    typeof round[3] !== "bigint" ||
    round[3] < round[2] ||
    round[3] > block.timestamp ||
    decimalsResult.result !== asset.decimals ||
    descriptionResult.result !== asset.description
  )
    return null;
  ...
  return {
    priceUsd,
    priceAnswer: String(round[1]),
    sourceRoundId: String(round[0]),
    sourceUpdatedAt: Number(round[3]),
  };
}
...
export async function readSourceSnapshot(client, { now = Date.now } = {}) {
  ...
  if (!Array.isArray(results) || results.length !== contracts.length)
    throw new Error("Incomplete BSC source observation");
...
export function publicSnapshot(snapshot, { now = Date.now } = {}) {
  ...
  if (!Array.isArray(snapshot.readings) || snapshot.readings.length !== ASSETS.length)
